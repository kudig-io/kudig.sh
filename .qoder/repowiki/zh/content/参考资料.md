...
## 简介
本文档围绕 kudig.sh 参考资料进行系统化整理，重点阐述其与相关组件的关联。核心内容包括：diagnose_k8s.sh 作为数据收集器，以 root 权限运行，负责采集系统、服务、网络、日志等全方位信息，并生成结构化的诊断目录，为 kudig.sh 提供离线分析所需的数据源；healthz.go 作为 Kubernetes 原生健康检查机制的实现，定义了 healthz.HealthChecker 接口、livez/readyz 端点及 AddHealthChecks 方法，代表了组件内部的实时健康探针。通过对比分析，本文旨在阐明 kudig.sh 的离线诊断与 healthz.go 的实时健康检查在运维体系中的互补关系，前者用于事后故障根因分析，后者用于即时状态监控。

## 项目结构
项目仓库包含以下关键文件，共同构成了诊断分析的完整链条：
- **reference/diagnose_k8s/diagnose_k8s.sh**：核心数据收集脚本，负责以 root 权限在 Kubernetes 节点上执行，收集系统、服务、网络、日志、容器运行时和 Kubernetes 组件等信息，并生成标准化的诊断目录。
- **reference/healthz.go**：Kubernetes 健康检查机制的 Go 语言实现，定义了健康检查接口、注册方法和端点，是组件内部实时健康状态的体现。
- **kudig.sh**：离线分析工具，消费 diagnose_k8s.sh 生成的诊断目录，通过预设规则进行多维度异常检测，并生成中英文对照的诊断报告。
- **README.md**：工具的使用说明、工作流程、异常检测规则和输出示例，是用户操作的主要指南。
- **TESTING.md**：测试方法与预期输出示例，用于验证工具链的正确性和稳定性。
...
## 核心组件
- **diagnose_k8s.sh**：作为数据收集器，必须以 root 权限运行，确保能访问所有系统关键信息和日志文件。其主要职责是收集系统、服务、网络、日志、容器运行时和 Kubernetes 组件等信息，并将这些信息组织成结构化的诊断目录，供后续的离线分析使用。
- **kudig.sh**：作为离线分析器，读取 diagnose_k8s.sh 生成的诊断目录，根据预设的规则进行异常检测，最终输出中英文对照的文本或 JSON 报告。其退出码（0、1、2）可用于自动化运维场景，指示节点的健康状态。
- **healthz.go**：作为 Kubernetes 健康检查机制的实现，提供了 healthz、livez、readyz 等端点的注册与处理功能。它体现了 Kubernetes 组件内部的实时健康检查能力，是集群自愈和探针机制的基础。
...
## 架构总览
下图展示了 kudig.sh 与 diagnose_k8s.sh、healthz.go 的核心关系：diagnose_k8s.sh 作为数据收集器，生成包含系统、网络、服务和日志信息的诊断目录；kudig.sh 作为离线分析器，消费该目录并进行事后分析；而 healthz.go 代表的实时健康检查则在组件内部持续运行，两者共同构成了 Kubernetes 节点的健康监控体系。
...
- **角色与权限**
  ...
  - 生成 `/tmp/diagnose_<timestamp>` 诊断目录，包含 `daemon_status`、`logs` 等子目录，便于后续分析。
- **关键函数与职责**
...
- **分析流程**
  ...
  - **环境检查**：校验必要命令（grep, awk, sed 等）是否存在。
